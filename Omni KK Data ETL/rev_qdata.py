...
def main(args):     
    ...
    # 3. date of updated data in format wwww.d
    #print(args)    #debug
    ETL_files = dQ.file_list();
    arch_f = "archive"
    cnxn = dQ.SQL_init();
    cursor = cnxn.cursor()
    dQ.logger(3, "started processing {} to update data on {}".format(args[1], args[3]))
    cursor.execute("SELECT ID FROM datetable WHERE \
        weeknum = " + args[3][0:4] + " AND dayofweek = " + args[3][-1])
    ...
    for file in ETL_files:
        ...
        for sheet in xlsx.sheetnames:
            ...
            print(icolumn_letter)
            print(PDdata)  #debug
            print(defects)
            dQ.insert_data(cursor, dateID, PDdata, defects)
        cnxn.commit()
        dQ.logger(0, "{} data on {} ({}) is successfully updated.".format(file, args[3], args[2]))
        xlsx.close()
